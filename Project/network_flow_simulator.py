            ...
            for i in range(samples):
                ...
                for node in self.graph.nodes():
                    if node != gateway_id and self.graph.has_edge(gateway_id, node):
                        ...
                        self.graph[node][gateway_id]['flow'] = traffic * 0.3 
    ...
    def edmonds_karp(self, source, sink, measure_performance=True):
        ...
        while True:
            ...
            if measure_performance:
                ...
                if path:
                    performance_data['path_lengths'].append(len(path) - 1)  
            if not path:
        ...
        for u, v, data in self.graph.edges(data=True):
            ...
            if capacity > 0:
                ...
                if flow > 0:
                    ...
                    edge_widths.append(1 + utilization * 4)  
